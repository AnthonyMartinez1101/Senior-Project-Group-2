...
namespace Modern2D
{
	...
	public class LightingSystem : MonoBehaviour
	{
        ...
        [HideInInspector] public Cryo<float> _2dLightsShadowStrength = new Cryo<float>(1f);
        ...
        [Tooltip("2d light shadow strength multiplayer")]
        [HideInInspector] public Cryo<float> _minimumAlphaOutOfLight = new Cryo<float>(0.1f);
        [HideInInspector] public Cryo<float> _maximumAlphaOutOfLight = new Cryo<float>(1f);
		...
		[Tooltip("Uses point lights for shadow projection")]
		[HideInInspector] public Modern2D.LightMode lightmode = Modern2D.LightMode.Directional;

		[Header("DEPENDENCIES")]

		[Tooltip("Material that determines how the shadows are rendered")]
		public const string shadowsLayer = "ModernShadows";
        public string AutoLightBoundsPrefKey = "AutoLightBoundsEnabled";
        public bool UsePresetsKey = false;
        ...
        [HideInInspector] public string PresetKey = "LightShadowPreset";
        [HideInInspector] public string StrengthRangeKey = "LightShadowStrengthRange";
        #endregion
        ...
        public void SetCallbacks()
		{
            ...
            _maximumAlphaOutOfLight.onValueChanged = OnShadowSettingsChanged;
		...
		public void OnShadowSettingsChanged()
		{
            ...
            Shader.SetGlobalFloat("_2DLightMaxAlpha", _maximumAlphaOutOfLight.value);
        ...
        public Modern2D.LightMode lightMode;
        ...
        public float AngleFromLightToShadow(StylizedShadowCaster shadow, out Vector3 lightpos) 
		{
            ...
            float dMin = distMinMax.value.x;
            float dMax = distMinMax.value.y;
            // cutoff: lights farther than this won't contribute
            float cutoff = dMax * 0.25f;

            // ensure farBoundary is strictly greater than near so InverseLerp behaves
            float farBoundary = Mathf.Max(cutoff, dMin + 0.0001f); // avoid division by zero if equal
            ...
            if (anglesIdx > -1)
            {
                ...
                for (int i = 0; i <= anglesIdx; i++)
                {
                    float ang = angles[i] * Mathf.Deg2Rad;
                    float w = inverseDistances[i];

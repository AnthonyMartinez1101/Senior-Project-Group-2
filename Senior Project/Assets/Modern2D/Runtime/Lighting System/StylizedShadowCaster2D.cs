...
namespace Modern2D
{
    ...
    public class StylizedShadowCaster2D : MonoBehaviour
    {
        ...
        public void Start()
        {
            ...
            if(LightingSystem.system !=null) CreateShadow();
            if (LightingSystem.system == null)
            {
                Debug.LogWarning("Shadows casters in scene but no LightingSystem found!");
            }
                // bug fix for version 2.0.1v 
                if (shadowData != null && rebuildShadowsBF_2_0_1_v)
            {
                rebuildShadowsBF_2_0_1_v = false;
                if (LightingSystem.system != null) _shadowData = CreateShadowData();
            }
